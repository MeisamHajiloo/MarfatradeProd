...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showError(form, message) {
        ...
        errorElement.style.cssText = 'color: #ff3860; margin-top: 10px; padding: 10px; background: #ffe6e6; border-radius: 4px;';
    ...
    async function registerUser(name, email, password) {
        try {
            ...
            showError(registerForm, error.message || 'An error occurred. Please try again.');
        }
    }
    ...
    function updateAuthUI(isLoggedIn, userData) {
        if (isLoggedIn && userData) {
            authLink.textContent = `Welcome, ${userData.name}`;
